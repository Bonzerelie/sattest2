document.addEventListener('DOMContentLoaded', () => {
  ...
  const scaleVisualization = document.getElementById('scale-visualization');
  ...
  const scaleData = {
    ...
    // Additional scales will continue in Part 2...
    "D": {
  ...
  function loadNewNote() {
    ...
    if (
      currentMode === 8 &&
      scaleData[currentScale].noteMap[scaleData[currentScale].noteOrder[0]].length === 2
    ) {
  ...
  function renderScaleKeyboard() {
    scaleVisualization.innerHTML = '';
    const keys = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];
    const scaleNotes = scaleData[currentScale].noteOrder;
    const tonic = scaleNotes[0];

    keys.forEach(note => {
      const div = document.createElement('div');
      div.classList.add('key');
      if (scaleNotes.includes(note)) div.classList.add('highlight');
      if (note === tonic) div.classList.add('tonic');
      div.textContent = note;
      scaleVisualization.appendChild(div);
    });
  }
